...
use App\Modules\MasterLocation\Villages\Requests\GetRequest;
...
class ApiVillagesController extends Controller
{
    public function get(GetRequest $request)
    {
        try {
            ...
            $data = Villages::with('districts.cities.provinces')->where($conditionalWhere);

            if ($request->province) {
                $data = $data->whereHas('districts.cities.provinces', function ($query) use ($request) {
                    $query->where('nama', 'like', "%$request->province%");
                });
            }

            if ($request->city) {
                $data = $data->whereHas('districts.cities', function ($query) use ($request) {
                    $query->where('nama', 'like', "%$request->city%");
                });
            }

            if ($request->district) {
                $data = $data->whereHas('districts', function ($query) use ($request) {
                    $query->where('nama', 'like', "%$request->district%");
                });
            }
            ...
            if ($safeRequest->sort) {
                ...
                if (!Schema::hasColumn('desa', $column)) {
