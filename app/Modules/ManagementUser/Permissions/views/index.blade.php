@extends('layout')
@section('title', 'Manajemen Pengguna: Permissions')
@push('style')
...
@section('content')
    <div class="container-fluid">
        @generate_navbar(h='/',t='Manajemen Pengguna';h='#',t="Permissions",tt="Permissions")
                    ...
                    <div class="card-body">
                        <div class="d-sm-flex d-block align-items-center justify-content-between mb-7">
                            <div class="mb-2 mb-sm-0">
                                <h5 class="card-title fw-semibold">Master Permissions</h5>
                            </div>
                        </div>
                        <div class="d-sm-flex d-block align-items-center justify-content-between mb-3">
                            ...
                            @if ($total_permissions !== 0)
                                <form id="generatePermissions"
                                    action="{{ route('management_user.permissions.generate-permissions') }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="_id" value="1">
                                </form>
                                <a href="javascript:void(0)"
                                    onclick="document.getElementById('generatePermissions').submit();"
                                    class="generate-permissions">
                                    <div
                                        class="btn bg-success-subtle text-success btn-md btn-icon btn-icon-start ms-1 mb-1">
                                        <i class="ti ti-upload mx-1 fs-3"></i> Generate Permissions
                                    </div>
                                </a>
                            @endif
                        </div>
                ...
                <form novalidate id="formData" method="post">
                    <div class="modal-body">
                        <div class="mb-2 form-group">
                            <label class="form-label">Nama Permissions
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $('#addBtn').on('click', function() {
                ...
                // Show the modal
                $('#modalDataTitle').text('Tambah Permission');
            ...
            datatable = $('.datatable').DataTable({
                ...
                ordering: true,
                ajax: {
                    url: "{{ route('management_user.permissions.datatable') }}",
                    type: 'GET',
            ...
            $(document).on('click', '.editBtn', function() {
                ...
                $.ajax({
                    url: `/management_user/permissions/${id}`,
                    type: 'GET',
                    success: function(response) {
                        if (response.success) {
                            ...
                            $('#modalDataTitle').text('Edit Permission');
                            $('#modalData').modal('show');
                        } else {
                            Swal.fire({
                                icon: 'error',
            ...
            $('#save').click(function(e) {
                ...
                let url = _id ? `/management_user/permissions/${_id}` :
                    `/management_user/permissions`; // If _id exists, it's an update
