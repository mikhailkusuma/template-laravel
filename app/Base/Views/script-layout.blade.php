<script>
    ...
    var localUserSettings = localStorage.getItem("userSettings");
    if (!localUserSettings) {
        // set the theme to storage
        localStorage.setItem("userSettings", JSON.stringify(defaultUserSettings));
    }

    // check if the theme is not null
    if (localUserSettings) {
        // parse the theme
        var splitUserSettings = JSON.parse(localUserSettings);

        // check if the theme is not null
        if (splitUserSettings.Theme !== null) {
            // set the theme
            defaultUserSettings.Theme = splitUserSettings.Theme;
        }

        // check if the direction is not null
        if (splitUserSettings.Direction !== null) {
            // set the direction
            defaultUserSettings.Direction = splitUserSettings.Direction;
        }

        // check if the colorTheme is not null
        if (splitUserSettings.ColorTheme !== null) {
            // set the colorTheme
            defaultUserSettings.ColorTheme = splitUserSettings.ColorTheme;
        }

        // check if the Layout is not null
        if (splitUserSettings.Layout !== null) {
            // set the layout
            defaultUserSettings.Layout = splitUserSettings.Layout;
        }

        // check if the BoxedLayout is not null
        if (splitUserSettings.BoxedLayout !== null) {
            // set the boxedLayout
            defaultUserSettings.BoxedLayout = splitUserSettings.BoxedLayout;
        }

        // check if the SidebarType is not null
        if (splitUserSettings.SidebarType !== null) {
            // set the SidebarType
            defaultUserSettings.SidebarType = splitUserSettings.SidebarType;
        }

        // check if the cardBorder is not null
        if (splitUserSettings.cardBorder !== null) {
            // set the cardBorder
            defaultUserSettings.cardBorder = splitUserSettings.cardBorder;
        }
    }

    // function to change the theme
    function changeTheme(params) {
        // get data current array user settings
        var localStorageUserSettings = JSON.parse(localStorage.getItem("userSettings"));
        // check if the array user settings is not null
        if (localStorageUserSettings) {
            // check if the Theme is not null
            if (localStorageUserSettings.Theme !== null) {
                // set the Theme
                localStorageUserSettings.Theme = params;
            }
        } else {
            // set the Theme
            localStorageUserSettings = {
                Theme: params
            };
        }
    ...
    function themeDirection(params) {
        // get data current array user settings
        var localStorageUserSettings = JSON.parse(localStorage.getItem("userSettings"));
        // check if the array user settings is not null
        if (localStorageUserSettings) {
            // check if the Direction is not null
            if (localStorageUserSettings.Direction !== null) {
                // set the Direction
                localStorageUserSettings.Direction = params;
            }
        } else {
            // set the Direction
            localStorageUserSettings = {
        ...
        // set the Direction to storage
        localStorage.setItem("userSettings", JSON.stringify(localStorageUserSettings));
    }

    function handleColorTheme(params) {
        $("html").attr("data-color-theme", params);
        $(params).prop("checked", !0);
        ...
        if (localStorageUserSettings) {
            // check if the ColorTheme is not null
            if (localStorageUserSettings.ColorTheme !== null) {
                // set the ColorTheme
                localStorageUserSettings.ColorTheme = params;
            }
        } else {
            // set the ColorTheme
            localStorageUserSettings = {
    ...
    function handleLayout(params) {
        ...
        if (localStorageUserSettings) {
            // check if the Layout is not null
            if (localStorageUserSettings.Layout !== null) {
                // set the Layout
                localStorageUserSettings.Layout = params;
            }
        } else {
            // set the Layout
            localStorageUserSettings = {
        ...
        // set the Layout to storage
        localStorage.setItem("userSettings", JSON.stringify(localStorageUserSettings));
    }

    function handleBoxedLayout(params) {
        // get data current array user settings
        var localStorageUserSettings = JSON.parse(localStorage.getItem("userSettings"));
        // check if the array user settings is not null
        if (localStorageUserSettings) {
            // check if the BoxedLayout is not null
            if (localStorageUserSettings.BoxedLayout !== null) {
                if (params === "boxed") {
                    // set the BoxedLayout
                    localStorageUserSettings.BoxedLayout = true;
                } else {
                    // set the BoxedLayout
                    localStorageUserSettings.BoxedLayout = false;
                }
            }
        } else {
            // set the BoxedLayout
        ...
        // set the BoxedLayout to storage
        localStorage.setItem("userSettings", JSON.stringify(localStorageUserSettings));
    }

    function handleSidebarType(params) {
        // get data current array user settings
        var localStorageUserSettings = JSON.parse(localStorage.getItem("userSettings"));
        // check if the array user settings is not null
        if (localStorageUserSettings) {
            // check if the SidebarType is not null
            if (localStorageUserSettings.SidebarType !== null) {
                // set the SidebarType
                localStorageUserSettings.SidebarType = params;
            }
        } else {
            // set the SidebarType
            localStorageUserSettings = {
        ...
        // set the SidebarType to storage
        localStorage.setItem("userSettings", JSON.stringify(localStorageUserSettings));
    }

    function handlecardBorder(params) {
        // get data current array user settings
        var localStorageUserSettings = JSON.parse(localStorage.getItem("userSettings"));
        // check if the array user settings is not null
        if (localStorageUserSettings) {
            // check if the cardBorder is not null
            if (localStorageUserSettings.cardBorder !== null) {
                if (params === "border") {
                    // set the cardBorder
                    localStorageUserSettings.cardBorder = true;
                } else {
                    // set the cardBorder
                    localStorageUserSettings.cardBorder = false;
                }
            }
        } else {
            // set the cardBorder
    ...
    var settings = Object.assign({}, defaultUserSettings);
